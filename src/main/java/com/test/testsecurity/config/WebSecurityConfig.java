package com.test.testsecurity.config;

import com.test.testsecurity.filter.JwtAuthenticationFilter;
...
@Configuration
// : 해당 클래스가 Spring의 설정 클래스로 사용됨을 명시 (Spring이 관리하는 객체를 생성하는데 사용)
@EnableWebSecurity
...
public class WebSecurityConfig {
    ...
    public AuthenticationManager authenticationManager(BCryptPasswordEncoder bCryptPasswordEncoder) throws Exception {
        ...
        // 비밀번호 검증을 휘한 bCryptPasswordEncoder 사용
        authProvider.setPasswordEncoder(bCryptPasswordEncoder);

        // ProviderManager를 반환 DaoAuthenticationProvider를 사용하여 인증 처리
        // - 다중 인증 Provider 관리를 반환 (사용자 인증 처리 관리자를 관리)
