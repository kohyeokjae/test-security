package com.test.testsecurity.provider;
...
/*
    ...
    - HS256 암호화 알고리즘을 사용하여 JWT 서명
      ...
      >> JWT 만료 기간은 10시간 저정 - 환경변수(jwt.expiration)
 */
...
public class JwtProvider {
    ...
    public Set<String> getRolesFromJwt(String token) {
        Claims claims = getClaims(token);
        List<String> roles = claims.get("roles", List.class); // roles 키로 수정
        if (roles == null) {
            return new HashSet<>(); // null일 경우 빈 Set 반환
        }
